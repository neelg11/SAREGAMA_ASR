#!/usr/bin/env python3
"""
Whisper Medium — Encoder-only LoRA finetuning for Singing ASR (Romanized Hindi).
Launch: torchrun --nproc_per_node=2 train_whisper_medium_encoder_lora.py
...
Fixes vs original:
  ...
  FIX-4  ddp_find_unused_parameters=False
         Gradient checkpointing + frozen decoder + DDP can deadlock with
...
OUTPUT_DIR       = "outputs/whisper-medium-enc-lora"
CACHE_DIR        = "cache/hf_datasets"

WANDB_PROJECT    = "whisper-singing-asr"
WANDB_RUN_NAME   = "medium-enc-lora-r32"

# LoRA — encoder ONLY via regex
LORA_CONFIG = LoraConfig(
    ...
    target_modules=r"model\.encoder\.layers\.\d+\.self_attn\.(q_proj|k_proj|v_proj|out_proj)",
)
...
TRAINING_ARGS = dict(
    ...
    ddp_find_unused_parameters=False,  # safe: all LoRA params flow encoder→decoder→loss
    report_to=["wandb"],
...
# ──────────────────────────────────────────────
# Verify LoRA only hits encoder
# ──────────────────────────────────────────────
_DECODER_RE = re.compile(r"model\.decoder\.")


def assert_encoder_only(model) -> None:
    for name, _ in model.named_parameters():
        if "lora_" in name and _DECODER_RE.search(name):
            raise RuntimeError(
                f"LoRA adapter found on decoder module: {name}."
            )
    logger.info("✓ LoRA adapters verified: encoder layers only.")
...
def main():
    ...
    # ── Apply LoRA ──────────────────────────────────────────────────────
    logger.info("Applying LoRA (encoder only) …")
    model = get_peft_model(model, LORA_CONFIG)
    assert_encoder_only(model)
    ...
    # gradient_checkpointing_enable() modifies the inner encoder layers —
    # both are orthogonal and order does not matter, but this is cleaner).
